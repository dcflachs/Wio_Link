...
class V2HandlersState():
    def __init__(self, tokens, conns):
        self.well_known = {}
        self.write_cache = {}
        self.events_cache = {}
...
class V2Listener():
    ...
    def __init__(self, state, tokens, conns):
        ...
        for row in self.tokens:
            self.well_known_subscriber(row)
            self.queued_write_subscriber(row)
            self.events_subscriber(row)
...
class myApplication(web.Application):
    ...
    def __init__(self):
        ...
        if use_v2_api:
            self.v2_state = V2HandlersState(get_all_token(), DeviceServer.accepted_xchange_conns)
            handlers = handlers + [
